...
export default function RelatedWord({ word, count }: { word: string, count: number }) {
    ...
    return <div onClick={() => {
        ...
        <p>{word}</p>
        <p className="absolute -top-3 -right-3 border border-white bg-black text-white rounded-full px-2 text-sm">{count}</p>
    </div>
